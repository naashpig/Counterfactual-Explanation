"""Module containing a template class to generate counterfactual explanations.
   ...
   All methods are in CFE.explainer_interfaces"""
...
from CFE.counterfactual_explanations import CounterfactualExplanations
...
class ExplainerBase:
    ...
    def do_posthoc_sparsity_enhancement(self, final_cfs_sparse, query_instance, posthoc_sparsity_param, posthoc_sparsity_algorithm):
        ...
        quantiles = self.data_interface.get_quantiles_from_training_data(quantile=posthoc_sparsity_param) # 0.1 입력 출력 age: 4 week:5
        mads = self.data_interface.get_valid_mads()
        for feature in quantiles:
            quantiles[feature] = min(quantiles[feature], mads[feature]) # 음 왜이렇게 하는거지?

        features_sorted = sorted(quantiles.items(), key=lambda kv: kv[1], reverse=True) # value 값을 기준으로 내림차순 정렬
        ...
        for cf_ix in range(len(final_cfs_sparse)):
            # current_pred = self.predict_fn_for_sparsity(final_cfs_sparse.iloc[[cf_ix]][self.data_interface.feature_names]) # iloc[[x]] 라는건 pandas 에서만 되는듯? 데이터 추출임 iloc[x]는 slicing이고-> 응용하면 iloc[[x],[y]] 이런식으로 되는거임
            # 4x9 final_cfs_sparse에서 label 제거하고 4x8의 각 행이 predict_fn_for_sparsity로 들어감
            for feature in features_sorted:
                current_pred = self.predict_fn_for_sparsity(final_cfs_sparse.iloc[[cf_ix]][self.data_interface.feature_names]) # 좀 충격이네;; 이 모듈 자체가 override 된 모듈이기 때문에 predict_fn_for_sparsity를 따라가도 dice_pytorch로 따라가야하는데 잘 못 따라감
                #feat_ix = self.data_interface.continuous_feature_names.index(feature)
            ...
            cfs_preds_sparse.append(temp_preds) # 바뀐 feature에 대해 prediction 결과 저장
    ...
    def do_linear_search(self, diff, decimal_prec, query_instance, cf_ix, feature, final_cfs_sparse, current_pred):
        ...
        if self.model.model_type == 'classifier':
            while((abs(diff)>10e-4) and (np.sign(diff*old_diff) > 0) and # np.sign항은 왜 들어간지 모르겠다
                ...
                final_cfs_sparse.loc[cf_ix, feature] += np.sign(diff)*change # 일종의 fine tuning이라고 생각하면 됨. original에서 -3된 CF가 생성되었으면, diff는 3이고 feature 값을 +1씩 하면서 original 제일 가까운 CF를 찾는다.
                current_pred = self.predict_fn_for_sparsity(final_cfs_sparse.iloc[[cf_ix]][self.data_interface.feature_names])
                old_diff = diff

                if(((self.target_cf_class == 0 and current_pred > self.stopping_threshold) or (self.target_cf_class == 1 and current_pred < self.stopping_threshold))): # label이 바뀌어버렸으면
