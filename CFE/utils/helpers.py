...
def new_labeler(model,dataset,label):
    labels=dataset[[label]].values
    dataset.drop(label, axis=1, inplace=True)
    new_labels = model(torch.FloatTensor(dataset.values))
    new_labels = torch.where(new_labels >= 0.5, 1, 0).numpy()
    dataset[label] = new_labels
    dataset=dataset[new_labels == labels].reset_index(drop=True)
    return dataset

def load_adult_income_dataset(only_train=True, model_based=False, model_path='',train_dataset=None,test_dataset=None):
    if model_based:
        model=load_model(model_path)
        train_dataset=new_labeler(model,train_dataset,'income')
        test_dataset=new_labeler(model,test_dataset,'income')
        ...
        adult_data = adult_data.replace({'workclass': {'Self-emp-not-inc': 'Self-Employed', 'Self-emp-inc': 'Self-Employed'}}) # 자영업자
...
def get_adult_income_data_info():
    ...
    outcome_name = 'income'
    return feature_description,continuous_features,outcome_name
...
def get_base_gen_cf_initialization(data_interface, encoded_size, cont_minx, cont_maxx, margin, validity_reg, epochs, wm1, wm2, wm3, learning_rate ):
    ...
    from CFE.utils.sample_architecture.vae_model import CF_VAE
