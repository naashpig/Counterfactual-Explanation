...
class Data:
    ...
    def decide_implementation_type(self, params):
        ...
        self.__class__  = decide(params) # 와 class를 바꿔버리는것도 있네 ㅋㅋ
        self.__init__(params) # 바뀐 class에 입력 넣어버리기!
...
def decide(params):
    ...
    if 'dataframe' in params: # if params contain a Pandas dataframe, then use PublicData class
        from CFE.data_interfaces.public_data_interface import PublicData
        return PublicData
    else: # use PrivateData if only meta data is provided
        from CFE.data_interfaces.private_data_interface import PrivateData
        return PrivateData
